...
function sliderInit(sliderClassName, navClassName) {

	// слайдер большая картинка
	$(sliderClassName).slick({
		slidesToShow: 1,
		slidesToScroll: 1,
		arrows: false,
		fade: true,
		asNavFor: navClassName
	});



	var $navSlider = $(navClassName);
	var paramCenterMode = true;
	var navSlidesCount = $navSlider.find('div').length;


	if(navSlidesCount <= 5){
		paramCenterMode = false;
	}
	...
	$navSlider.slick({
		slidesToShow: 5,
		slidesToScroll: 5,
		asNavFor: sliderClassName,
		centerMode: paramCenterMode,
		centerPadding: '0px',
		focusOnSelect: true,
		arrows: false,
	});
...
function sliderInitOnTab($slider, $navSlider, sliderClassName, navClassName) {
// слайдер большая картинка
	$slider.slick({
		slidesToShow: 1,
		slidesToScroll: 1,
...
$(document).ready(function() {
	...
	sliderInit('.slider-for1', '.slider-nav1');
	// sliderInit('.slider-for2', '.slider-nav2');


	$(".js-form").submit(function(e) {
		var fieldData = $(this).find('input[name=was]:checked').val();

		if(fieldData==='yes') {
			e.preventDefault();

			$.fancybox( $('#modal-sorry'), {
				padding: 0,
				fitToView: false
			} );
	...
	var $switchers = $('.js-switcher');
	var $switcherTab = $('.js-switcher-tab');
	// var id = $switchers.eq(0).attr('href');
	...
	$switchers.click(function(e) {
		...
		if ($tab!=undefined) { // если в предыдущей вкладке слайдер инициировался, то разрушим предыдущий слайдер
			$tab.find('.slider-for2').slick('unslick');
			$tab.find('.slider-nav2').slick('unslick');
			$tab.hide(); // после разрушения скрываем вкладку
		}
		...
		$tab.fadeIn();		 // показываем вкладку

		var $slider = $tab.find('.slider-for2');
		var $sliderNav =$tab.find('.slider-nav2');

		sliderInitOnTab($slider, $sliderNav, '#'+$slider.attr('id'), '#'+$sliderNav.attr('id')); // инициируем карусель
...
function initialize() {
	...
	var styledMap = new google.maps.StyledMapType(styles,
		{name: "Styled Map"});
	var myLatlng = new google.maps.LatLng(55.704251, 37.615892);
	var mapOptions = {
		zoom: 16,
		zoomControl: false,
		scaleControl: false,
		scrollwheel: false,
		center: new google.maps.LatLng(55.704251, 37.624292),
